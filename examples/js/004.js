...
ex1 = (function() {
    ...
    osc1_freq = T("glide", 5, 880);
    osc2_freq = T("glide", 5, 880);
    
    osc1 = T("osc", "pulse", osc1_freq, 0.25);
    osc2 = T("osc", "tri"  , osc2_freq, 0.50);
    vco  = T("+", osc1, osc2);
    
    env1 = T("adsr", 50, 100, 0.5, 0, 2000);
    vcf  = T("rlpf", vco);
    vcf.cutoff = T("+", 2300, env1).kr();
    
    env2 = T("adsr", 0, 200, 1);
    vca  = T("*", vcf, env2);
    
    efx1 = T("efx.dist"  , 0, 0, 8000, vca).off();    
    efx2 = T("efx.chorus", efx1);
    efx3 = T("efx.delay" , efx2);
    ...
    metro = T("interval", timbre.utils.bpm2msec(80, 16), function() {
        var note = score[metro.count % score.length];
        osc1.freq.value = m64tof(note + osc1.$.fine) / osc1.$.freq;
        osc2.freq.value = m64tof(note + osc2.$.fine) / osc2.$.freq;
        env1.bang();
        env2.bang();
    });
...
function initUI() {
    ...
    ui.ctl.set({type:"knob" , x:25, y:120, radius:15, value:5,
                change:function() {
                    osc1_freq.duration = (this.value/2) * 5;
                    osc2_freq.duration = (this.value/2) * 5;
                }});
